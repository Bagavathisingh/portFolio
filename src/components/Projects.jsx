        ...
        <section id="projects" className="bg-[#050505] font-mono border-t border-white/5 relative">
            ...
            <div className="flex border-b border-white/5 bg-white/[0.01]">
                <div className="flex-1 p-8 md:p-12">
                    <div>
                        <h2 className="text-6xl md:text-7xl font-black tracking-tighter uppercase leading-none text-white">PROJECTS_</h2>
                    </div>
                </div>
            </div>

            {/* Project List */}
            <div className="flex flex-col relative z-10 transition-all duration-700">
                        ...
                        <div className="flex flex-col lg:flex-row min-h-[140px]">
                            ...
                            <div className="flex-1 p-8 flex flex-col lg:flex-row lg:items-center justify-between gap-8">
                                <div className="max-w-2xl">
                                    ...
                                    <p className="text-xs text-gray-500 uppercase tracking-widest leading-relaxed max-w-xl group-hover:text-gray-300 transition-colors">
                                        {project.description}
                                    </p>
                                </div>
                                ...
                                <div className="flex flex-wrap gap-2 lg:flex-nowrap">
                                    {project.tech.map(t => (
                                        <div key={t} className="px-3 py-1 border border-white/5 bg-black text-[8px] font-bold text-gray-600 uppercase tracking-widest group-hover:border-neon/30 transition-colors">
                                            {t}
                                        </div>
                                    ))}
                                </div>
                            ...
                            {/* Action Links */}
                            <div className="lg:w-64 border-l border-white/5 p-8 flex lg:flex-col justify-center gap-6">
                                <a href={project.live} target="_blank" rel="noreferrer" className="flex items-center gap-4 group/btn">
                                    <div className="w-8 h-8 border border-white/10 flex items-center justify-center group-hover/btn:bg-neon group-hover/btn:border-neon transition-all">
                                        <ArrowUpRight size={14} className="group-hover/btn:!text-black transition-colors" />
                                    </div>
                                    <span className="text-[10px] font-black tracking-[0.2em] text-gray-500 group-hover/btn:!text-white transition-colors uppercase">Live Demo</span>
                                </a>
                                <a href={project.github} target="_blank" rel="noreferrer" className="flex items-center gap-4 group/btn">
                                    <div className="w-8 h-8 border border-white/10 flex items-center justify-center group-hover/btn:bg-white group-hover/btn:border-white transition-all">
                                        <Github size={14} className="group-hover/btn:!text-black transition-colors" />
                                    </div>
                                    <span className="text-[10px] font-black tracking-[0.2em] text-gray-500 group-hover/btn:!text-white transition-colors uppercase">View Code</span>
                                </a>
                            </div>

                            {/* Hover Dynamic Background (Right side overlay) */}
                            <div className={`absolute inset-y-0 right-0 w-[40%] pointer-events-none overflow-hidden transition-all duration-700 ease-in-out z-0 opacity-0 group-hover:opacity-10 ${hoveredIndex === index ? 'translate-x-0' : 'translate-x-20'
                                }`}>
                                <img
                                    src={project.image}
                                    alt={project.title}
